...
from app.services.claude_prompt import auth_error, auth_summary, build_agent_env, build_prompt
...
def test_sdk_message_logging_keeps_complete_content(
    ...
    try:
        with caplog.at_level(logging.DEBUG, logger="app.services.claude_event_adapter"):
            ClaudeEventAdapter()._log_message(assistant)
    finally:
...
def test_skill_scan_caches_parsed_metadata_and_refreshes_on_change(
    ...
    skill_dir = _write_skill(skills_root / "system", "demo", name="Demo", description="first")
    service = ClaudeRuntime(skills_root, tmp_path)
...
def test_native_skill_mapping_preserves_user_override(tmp_path: Path):
    ...
    other = _write_skill(skills_root / "system", "other", name="Other", description="other")
...
def test_client_pool_reuses_connection_and_reset_disconnects(
    ...
    async def exercise():
        first, warm, _ = await service.sessions.acquire(
            options=object(),
            user_id="u1",
            session_id="s1",
            signature=("rev1",),
        ...
        assert not warm
        assert first.client.sdk.connected
        await service.sessions.release(first, healthy=True)

        second, warm, _ = await service.sessions.acquire(
            options=object(),
            user_id="u1",
            session_id="s1",
            signature=("rev1",),
        ...
        await service.sessions.release(second, healthy=True)

        service.reset_session("s1")
        if service.sessions._cleanup_tasks:
            await next(iter(service.sessions._cleanup_tasks))
...
def test_reset_user_evicts_only_that_users_clients(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    clients = []
    ...
    async def exercise():
        first, _, _ = await service.sessions.acquire(
            options=object(), user_id="u1", session_id="s1", signature=("rev",)
        )
        second, _, _ = await service.sessions.acquire(
        ...
        if service.sessions._cleanup_tasks:
            await asyncio.gather(*tuple(service.sessions._cleanup_tasks))
        assert clients[0].disconnected
        assert not clients[1].disconnected
        assert list(service.sessions.entries) == [("u2", "s2")]
...
def test_agent_auth_requires_noninteractive_credential():
    ...
    assert auth_error({"ANTHROPIC_API_KEY": "key"}) is None
    assert auth_error({"CLAUDE_CODE_USE_BEDROCK": "1"}) is None
    assert auth_summary(
        {
