...
> 设计基线：Manifest V3，版本 `0.1.2`；自动更新 MVP 目标版本 `0.1.3`
> 最后核对：2026-07-27

> MVP 实现说明（2026-07-28）：首版暂不接对象存储，CRX 直接只读挂载到独立更新服务，由该服务同时提供 Update XML 和版本化 CRX 下载；本文对象存储章节保留为后续扩展设计。
...
- 扩展使用云端 Claude Code，正式发布 ID 必须进入后端 CORS 白名单。
...
生产、日常开发建议使用不同 ID，避免开发构建误用正式后端白名单。如果测试环需要验证真实升级，应使用同一生产 ID 和同一签名密钥，但仅在受保护 CI 产生 CRX，不把私钥下发给开发机。
...
2. 聊天流式响应、文件上传或权限确认进行中时，不调用 `chrome.runtime.reload()`。
...
当前扩展把界面偏好和设置放在 `localStorage`/`chrome.storage.local`。自动更新必须保证：
...
- 后端 CORS 必须包含生产 ID；开发 ID 与生产 ID 分开登记。
...
3. 生成新签名密钥和新扩展 ID。
4. 更新企业策略、后端精确 CORS、监控和 Update API 产品登记。
...
- 固定生产扩展 ID 后，后端 CORS 白名单不再使用不必要的通配范围。
