...
| 原状 | 本次改动 | 实际边界 |
| --- | --- | --- |
| 新会话仅发送 `hi` 仍加载完整默认提示与工具描述，实测 24.5 秒、$0.1151，且返回冗长项目介绍 | 默认启用 Claude Code 原生 `CLAUDE_CODE_SIMPLE_SYSTEM_PROMPT=1`，并复用常驻 SDK client；所有回答仍由所选模型生成 | 不引入基于文案的硬编码路由；冷启动和模型生成耗时仍存在，应按 first_text_ms 分开观测 |
...
| 模型固定 | `backend/config/models.yaml` 维护 GLM/DeepSeek 及各自 provider；选择器位于聊天框下方并按会话持久化；后端验证及队列传递 | 切换只允许发生在会话未执行时；会清除旧模型 SID，再由完整历史恢复 |
...
```dotenv
# 默认读取 backend/config/models.yaml；也可挂载其它目录文件
CLAUDE_MODELS_FILE=/app/config/models.yaml
# models.yaml 的 provider.*_env 可引用这些变量
GLM_BASE_URL=https://glm-compatible.example/anthropic
GLM_API_KEY=replace-me
DEEPSEEK_BASE_URL=https://deepseek-compatible.example/anthropic
DEEPSEEK_AUTH_TOKEN=replace-me
...
- 真实模型单会话烟测（样本使用 glm-5.2）：首轮正文约 7.35 秒；同一原生客户端第二轮约 1.73 秒；手动 compact 完成约 28.03 秒，压缩后正确返回先前标记，首字约 1.62 秒。这是单次样本，不是改造前后基准或生产 P95。
- Docker 实测另见下方补充记录。
