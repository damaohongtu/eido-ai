...
import { PROJECT_MATERIAL_ACCEPT } from '../utils/projectFiles';
...
const ProjectView: React.FC<ProjectViewProps> = ({
  ...
  const confirmDeleteProject = async () => {
    if (!window.confirm(`删除项目「${project.name}」？项目内会话会保留并移至未归属对话。`)) return;
    await onDelete();
  };

  return (
    <div className="flex h-full min-w-0 flex-col bg-gray-50/40">
      <header className="flex min-h-20 items-center justify-between gap-4 border-b border-gray-200 bg-white px-8 py-4">
        <div className="min-w-0">
          <div className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400">Project</div>
          <h1 className="truncate text-xl font-black text-gray-900">{project.name}</h1>
          {project.description ? <p className="mt-1 truncate text-sm text-gray-500">{project.description}</p> : null}
        </div>
        <button
          onClick={onNewChat}
          disabled={Boolean(project.archived_at)}
          className="shrink-0 rounded-xl bg-gray-800 px-5 py-2.5 text-sm font-bold text-white hover:bg-gray-900 disabled:cursor-not-allowed disabled:opacity-40"
        >
              ...
              <div className="mt-5 overflow-hidden rounded-xl border border-gray-200">
                {filesLoading ? (
                  ...
                  <div key={file.id} className="flex items-center gap-3 border-b border-gray-100 px-4 py-3 last:border-0">
                    ...
                    <a href={getProjectFileUrl(project.id, file.id)} target="_blank" rel="noreferrer" className="rounded-lg px-2 py-1.5 text-xs font-bold text-gray-500 hover:bg-gray-100">打开</a>
