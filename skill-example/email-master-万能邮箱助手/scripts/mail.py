...
def cmd_send(args):
    """发送邮件"""
    try:
        if not args.content and not args.html:
            print("错误: 需指定 --content 或 --html", file=sys.stderr)
            sys.exit(1)
        if args.content and args.html:
            print("错误: --content 与 --html 二选一", file=sys.stderr)
            sys.exit(1)
...
"""
    ...
    send_parser.add_argument('--content', help='邮件正文（纯文本）。与 --html 二选一')
    send_parser.add_argument('--html', help='HTML 文件路径，正文以 HTML 展示')
